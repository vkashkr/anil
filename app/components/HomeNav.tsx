...
export default function HomeNav({ initialProfilesById }: HomeNavProps) {
  ...
  const navigateToProfile = (name: string) => {
    router.push(`/ahmedabad-escort/${encodeURIComponent(name.trim().toLowerCase().replace(/\s+/g, '-'))}`);
    setSearchQuery('');
    setSearchFocused(false);
  };
      ...
      <div className="max-w-6xl mx-auto bg-black/30 backdrop-blur-xl rounded-2xl px-4 py-2.5 sm:px-5 sm:py-3 border border-white/10 shadow-2xl">
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="hidden md:flex items-center gap-4 text-sm flex-shrink-0">
            <a href="/ahmedabad-escort" className="text-gray-300 hover:text-pink-300 transition flex items-center gap-1">
        ...
        {mobileMenuOpen && (
          <div className="md:hidden mt-2 pt-2 border-t border-white/10 grid grid-cols-4 gap-2">
            <a href="/ahmedabad-escort" onClick={() => setMobileMenuOpen(false)} className="flex flex-col items-center gap-1 py-2 rounded-lg hover:bg-white/10 transition">
