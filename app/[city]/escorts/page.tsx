...
export default async function CityEscortPage({ params }: { params?: any }) {
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: `${cityDisplay} Escort`, item: `${BASE_URL}/${cityPath}/escorts` },
    ],
  };
  ...
  const itemListJsonLd = {
    '@context': 'https://schema.org',
    '@type': 'ItemList',
    name: `${cityDisplay} Escort Profiles`,
    url: `${BASE_URL}/${cityPath}/escorts`,
    numberOfItems: profiles.length,
    itemListElement: profiles.slice(0, 50).map((p, i) => ({
      '@type': 'ListItem',
      position: i + 1,
      url: `${BASE_URL}/${cityPath}/escorts/${encodeURIComponent(getProfileSlug(p))}`,
      name: p.name,
    })),
  };
  ...
  const faqJsonLd = {
    ...
    mainEntity: [
      {
        '@type': 'Question',
        name: `Are you searching for ${cityDisplay} escorts?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `I have real profiles that provide you escort service in ${cityDisplay}, India`,
        },
      },
      {
        '@type': 'Question',
        name: `How do I book a ${cityDisplay} escort?`,
        acceptedAnswer: {
          '@type': 'Answer',
      ...
      {
        '@type': 'Question',
        name: `Are the ${cityDisplay} escort profiles real and verified?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `Yes. All profiles on Aliya Escort ${cityDisplay} are genuine, independent adults with verified photos. No fake or stock images.`,
        },
      },
      {
        '@type': 'Question',
      ...
      {
        '@type': 'Question',
        name: `Is incall and outcall available in ${cityDisplay}?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `Yes. Both incall (at the escort\'s location) and outcall (hotel or home delivery) services are available 24/7 in ${cityDisplay}.`,
        },
      },
      {
        '@type': 'Question',
        name: `What is the rate for call girls in ${cityDisplay}?`,
        acceptedAnswer: {
          '@type': 'Answer',
    ...
    <div className="min-h-screen bg-zinc-950 text-gray-100 font-sans pb-14 md:pb-0">
      ...
      <nav className="sticky top-0 z-50 bg-black/70 backdrop-blur-md border-b border-white/10 px-4 py-3">
        <div className="max-w-5xl mx-auto flex items-center gap-3">
          ...
          <div className="flex items-center gap-1 text-sm text-gray-400">
            ...
            <span className="text-pink-300 font-semibold">{cityDisplay} Escort</span>
          </div>
        </div>
      </nav>
      ...
      <div className="max-w-5xl mx-auto px-3 py-5 space-y-4">
        {profiles.length === 0 ? (
          ...
          profiles.map((p, profileIndex) => {
            const profileSlug = getProfileSlug(p);
                ...
                <div className="flex flex-col shrink-0 w-[130px] sm:w-[180px] md:w-[210px]">
                  ...
                  <Link href={profileUrl} className="block relative overflow-hidden" style={{ height: '210px' }}>
                    {mainImage ? (
                      ...
                      <img
                        src={mainImage}
                        alt={`${p.name} escort in ${cityDisplay}`}
      ...
      <div className="max-w-3xl mx-auto px-4 py-6 border-t border-white/5">
        <h2 className="text-lg font-bold text-white mb-4">
            Frequently Asked Questions — {cityDisplay} Escort Service
          </h2>
        <div className="space-y-3">
            {[
              {
                q: `How do I book a ${cityDisplay} escort?`,
              ...
              },
              {
                ...
                a: `Yes. All profiles on Aliya Escort ${cityDisplay} are genuine, independent adults with verified photos. No fake or stock images.`,
              },
              {
                q: 'Is incall and outcall service available?',
                a: `Both incall (at the escort\'s location) and outcall (hotel or home delivery) services are available 24/7 across ${cityDisplay}.`,
              },
              {
      ...
      <div className="max-w-3xl mx-auto px-4 py-8 text-center border-t border-white/5">
        <h2 className="text-sm font-semibold text-gray-500 mb-2">About {cityDisplay} Escort Service</h2>
